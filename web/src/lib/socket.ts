...
export function getSocket(): Socket {
  if (!socket) {
    // In dev or prod, connect to same host or services port
    const socketUrl = import.meta.env.VITE_API_URL || (window.location.port === '3000' ? 'http://localhost:4000' : window.location.origin);
