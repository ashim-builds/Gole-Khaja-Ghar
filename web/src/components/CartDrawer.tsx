...
function CartItemCard({ 
  ...
  } else if (item.variantName && item.variantPrice) {
    itemPrice = item.variantPrice * item.qty;
    unitDisplay = item.variantName;
    basePriceDisplay = `Rs. ${item.variantPrice} each`;
  } else {
    itemPrice = (item.product.pricePerKg || 0) * item.qty;
    unitDisplay = "Portion";
    basePriceDisplay = `Rs. ${item.product.pricePerKg || 0}`;
  }
