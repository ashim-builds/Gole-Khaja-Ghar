...
export default function NotificationBell({ type }: NotificationBellProps) {
  ...
  const handleSubscribe = async () => {
    ...
    try {
      ...
      const subscribeRes = await api.push.subscribe(subJson, type);
      if (!subscribeRes.success) throw new Error("Failed to register subscription on server");
            ...
            <div className="py-4 space-y-3">
              {permission === "denied" ? (
                ...
                fetchingNotifications ? (
                  ...
                  <div className="max-h-72 overflow-y-auto space-y-2 pr-1 custom-scrollbar">
                    {notifications.map((notif) => {
                      ...
                      const targetUrl = type === "admin" 
                        ? (notif.orderId ? `/admin/orders/${notif.orderId}` : "/admin/orders")
                        : (notif.orderId ? `/orders/${notif.orderId}` : "/orders");
                      ...
                      return (
                        <Link
                          ...
                          className={`block rounded-xl p-3 border transition-all text-left border-l-4 border-amber-500 bg-amber-500/5 hover:bg-amber-500/10 ${
                ...
                <p className="text-xs text-stone-400 leading-normal">
                  Subscribe to receive native push notifications on status changes, new orders, and updates.
                </p>
              )}
            </div>
