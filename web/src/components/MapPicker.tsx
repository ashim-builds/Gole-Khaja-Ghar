...
export default function MapPicker({ onAddressSelect, initialAddress }: MapPickerProps) {
  ...
  // Default: Pokhara, Nepal (closer to the shop)
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Prevent race condition in React Strict Mode
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        ...
        if (err.code === err.PERMISSION_DENIED) {
          ...
          setLocationError({ message: "Location unavailable on this device. Please tap on the map to pin your address." });
        } else if (err.code === err.TIMEOUT) {
          setLocationError({ message: "Location request timed out. Please try again or tap the map manually." });
        } else {
      ...
      <div className="relative rounded-2xl overflow-hidden border border-stone-200 shadow-sm isolate" style={{ height: "300px" }}>
        ...
        <div className="absolute top-3 right-3 z-[1000] flex flex-col gap-2">
          <button
            ...
            className="flex items-center gap-2 bg-white text-black text-sm font-bold px-3 py-2 rounded-xl shadow-lg border border-stone-200 hover:bg-stone-50 transition-colors disabled:opacity-60"
          >
            {isLocating ? (
          ...
          <div className="flex-1">
            ...
            {locationError.showHelp && (
              <p className="text-xs text-amber-600 mt-1 font-medium">
                You can also <strong>tap anywhere on the map</strong> to manually set your delivery pin.
              </p>
            )}
          </div>
          <button
